...
ltcmt = function(data, replication, line, tester, traits, block)
  ...
  if(!missing(block)){
      ...
      z0 = function(x){
        ...
        model1 <- aov(var ~ Replication +  Replication:Block + Genotype, data = x)
      ...
      cmat
      res = list(Mean = indmean, ANOVA = anv, GCA.Line = lgca, GCA.Tester = tgca, SCA = SCA,
      ...
      z000 = function(x){
        Replication = as.factor(x[,2])
        Line = as.factor(x[,3])
        Tester = as.factor(x[,4])
        dat = data.frame(Replication, Line, Tester, var = x[,1])
      }
      dat = lapply(az, z000)
      ##################
      ...
      z0 = function(x){
        ...
        model1 <- aov(var ~ Replication +   Genotype, data = x)
        matrix1 <- as.matrix(anova(model1))
      }
      f0 = lapply(dat, z0)
      f0
